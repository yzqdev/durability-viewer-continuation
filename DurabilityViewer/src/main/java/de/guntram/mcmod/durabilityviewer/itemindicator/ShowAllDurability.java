...
public class ShowAllDurability implements IItemDecorator {
    ...
    @Override
    public boolean render(GuiGraphicsExtractor guiGraphics, Font font, ItemStack itemStack, int xPosition, int yPosition) {
        // 基础显示开关检查
        ...
        if (Config.showExpectedHits && Minecraft.getInstance().level != null) {
            try {
                ...
                var unbreakEnchant = access.lookupOrThrow(Registries.ENCHANTMENT)
        ...
        // 绘制文字（带阴影更清晰）
        guiGraphics.text(font, string, x, y, ARGB.opaque(color) );
